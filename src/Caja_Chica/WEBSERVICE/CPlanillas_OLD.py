...
class CPlanillas(CPlanillaBase):
    ...
    #----------------------------------------------
    # Realiza la apertura de planilla            OJOFPM PARECE QUE NO SE USA
    #----------------------------------------------
    def omApertura(self):
    ...
    def mxGratificacion(self, p_cCodigo, p_cCodEmp):
        ...
        for XPAG in RS:
            ...
            if lcCodigo != None:
               ...
               RS = self.loSql.omExecRS("SELECT SUM(nMonto) FROM ADM.E01DTRX WHERE cCodigo = '" + lcCodigo + "' AND cConcep = 'A03'")
               lnComisi += RS[0][0]
        lnGratif = lnComisi / 6
