...
//-------------------------------------
class CCChica extends CBase {
   public $paNomEnt, $paEstado, $plNuevo, $paData;
   ...
   public function omInitCuenta() {
      ...
      /*$RS = pg_query($lcSql);
      print_r($RS);
      $this->paNomEnt = pg_fetch_array($RS);*/
      $RS = $loSql->omExec($lcSql);
      $this->paNomEnt = $loSql->fetch($RS);
      return true;
   }

   public function omBuscarEntidad(){
      if (empty($this->paData['CCODENT'])) {
         $this->pcError = 'NO HA DEFINIDO CRITERIO DE BUSQUEDA';
         return false;
      }
      //die($this->paData['CCODENT'].'asdasd');
      ...
      $lcSql = "SELECT cNomEnt, cEntida, cNroDni FROM V_E02MPER WHERE cEntida = '$lcCodEnt'"; 
      $RS= $loSql->omExec($lcSql);
      $laFila = $loSql->fetch($RS);

      if (empty($laFila[0])) {
         $loSql->omDisconnect();
         $this->pcError = 'DNI ['.$lcCodEnt.'] NO EXISTE';
         return false;
      }
      $this->paData = array('CNOMENT' => $laFila[0], 'CCODENT' => $laFila[1], 'CNRODNI' => $laFila[2]);
      $loSql->omDisconnect();
      return $llOk;     
   }
   
   public function omAbrirCaja(){
      $llOk = $this -> mxValAbrirCaja();
      if(!$llOk){
         return false;
      }
      // Conexion DB
      ...
      $llOk = $this->mxAbrirCaja($loSql);
      $loSql->omDisconnect();
      return $llOk;
   }

   public function mxValAbrirCaja(){
   ...
   public function mxAbrirCaja($p_oSql){
      ...
      $lcSql = "INSERT INTO E02MRCH (cIdenti, cTipo, cNroDni, dFecReg, cEstado, cGlosa , cEntida, nMonto, cMoneda, cCodUsu, tModifi)
               VALUES ('$lcIdenti','R','$lcNroDni','$ldFecha','A','$lcGlosa','$lcCodEnt','00','S','$lcCodUsu',NOW())";
